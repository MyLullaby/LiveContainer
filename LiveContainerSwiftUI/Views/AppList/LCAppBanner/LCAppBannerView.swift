...
final class LCAppBannerRootView: UIView {
    ...
    func update(
        ...
        let isDark = iconStyle == 1 || (iconStyle == 2 && traitCollection.userInterfaceStyle == .dark)
        let icon = appInfo.iconIsDarkIcon(isDark) ?? UIImage()
        let mainColor = Self.extractMainHueColor(appInfo: appInfo, icon: icon, iconStyle: iconStyle, traitCollection: traitCollection)
    ...
    private static func extractMainHueColor(appInfo: LCAppInfo, icon: UIImage, iconStyle: Int, traitCollection: UITraitCollection) -> UIColor {
        let isDark = iconStyle == 1 || (iconStyle == 2 && traitCollection.userInterfaceStyle == .dark)
        if isDark, let cachedColor = appInfo.cachedColorDark {
            return cachedColor
        }
